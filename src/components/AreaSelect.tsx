...
function childrenOf(map: AreaMap, parentCode: string, level: 1 | 2) {
    ...
    return Object.entries(map).filter(([code]) => code.startsWith(prefix) && code !== '000000')
}

class AreaSelectImpl extends Component<
    AreaSelectProps,
    { open: boolean; provinceCode: string; cityCode: string }
> {
    state = { open: false, provinceCode: '', cityCode: '' }
    ...
    open = () => this.setState({ open: true, provinceCode: '', cityCode: '' })
    close = () => this.setState({ open: false })

    pickProvince = (code: string) => this.setState({ provinceCode: code })
    pickCity = (code: string) => this.setState({ cityCode: code })
    ...
    render() {
        ...
        const { open, provinceCode, cityCode } = this.state
        ...
        return (
            <>
                <View className="flex flex-row items-center justify-between border-b border-border px-4 py-3">
                    ...
                    <Button variant="ghost" size="sm" onClick={this.open}>
                        {this.displayName || '请选择地区'}
                    </Button>
                </View>

                <Drawer open={open} onOpenChange={value => this.setState({ open: value })}>
...
export const AreaSelect = observer(AreaSelectImpl) as typeof AreaSelectImpl
