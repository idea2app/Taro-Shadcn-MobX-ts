...
class ScrollListImpl<T extends DataObject = DataObject> extends Component<ScrollListProps<T>> {
    componentDidMount() {
        this.props.store.getList(this.props.filter, 1)
    }

    componentDidUpdate(previousProps: ScrollListProps<T>) {
    ...
    render() {
        ...
        return (
            <ScrollView className={className} style={style} scrollY onScrollToLower={this.loadMore}>
                {renderList(allItems)}
...
export const ScrollList = observer(ScrollListImpl) as typeof ScrollListImpl
